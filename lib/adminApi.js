...
export function updateAdminOrderInvoiceSent(token, orderNumber, sent) {
  return adminFetch(`/api/admin/orders/${encodeURIComponent(orderNumber)}/invoice-sent`, token, {
    method: 'PATCH',
...
export function fetchAdminExpenses(token, { period, from, to, limit } = {}) {
  const params = new URLSearchParams();
  ...
  if (limit != null) params.set('limit', String(limit));
  const qs = params.toString();
  return adminFetch(`/api/admin/expenses${qs ? `?${qs}` : ''}`, token, { method: 'GET' });
}

export function fetchAdminExpenseSummary(token) {
  return adminFetch('/api/admin/expenses/summary', token, { method: 'GET' });
}

export function createAdminExpense(token, payload) {
  return adminFetch('/api/admin/expenses', token, {
    method: 'POST',
    body: JSON.stringify(payload)
  });
}

export function deleteAdminExpense(token, id) {
  return adminFetch(`/api/admin/expenses/${encodeURIComponent(id)}`, token, {
    method: 'DELETE'
  });
}

export function fetchAdminSettings(token) {
  return adminFetch('/api/admin/settings', token, { method: 'GET' });
}
...
// --- Drea ledger (hairstyling business, separate from EVOQ books) ---

export function fetchAdminDreaEntries(token, { period, type, from, to, limit } = {}) {
  const params = new URLSearchParams();
  if (period) params.set('period', period);
  ...
  if (limit != null) params.set('limit', String(limit));
  const qs = params.toString();
  return adminFetch(`/api/admin/drea${qs ? `?${qs}` : ''}`, token, { method: 'GET' });
}

export function fetchAdminDreaSummary(token) {
  return adminFetch('/api/admin/drea/summary', token, { method: 'GET' });
}

export function createAdminDreaEntry(token, payload) {
  return adminFetch('/api/admin/drea', token, {
    method: 'POST',
    body: JSON.stringify(payload)
  });
}

export function deleteAdminDreaEntry(token, id) {
  return adminFetch(`/api/admin/drea/${encodeURIComponent(id)}`, token, {
    method: 'DELETE'
  });
}
