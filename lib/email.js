...
/**
 * Render the payment instructions as a complete block so card orders can never
 * inherit the Venmo button from the static EmailJS template.
 */
export function renderBuyerPaymentHtml({
  isCardOrder,
  total,
  venmoUsername,
  venmoUrl,
  ...
  const safeVenmoUrl = escapeHtml(venmoUrl);
  const safeOrderNumber = escapeHtml(orderNumber);
  const safeCustomerPhone = escapeHtml(customerPhone);

  if (isCardOrder) {
    const deliveryTiming = safeCustomerPhone
      ? `We'll send it to <strong>${safeCustomerPhone}</strong> within one business day.`
      : "We'll send it within one business day.";

    return (
      '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f5f1e9;border-radius:10px;margin-bottom:28px;">' +
        '<tr>' +
          '<td style="padding:24px;">' +
            '<h2 style="margin:0 0 8px 0;font-size:14px;letter-spacing:0.08em;text-transform:uppercase;color:#6b5f52;font-weight:600;">' +
              'Next step — check your texts' +
            '</h2>' +
            '<p style="margin:0 0 10px 0;font-size:15px;line-height:1.6;color:#2f2a25;">' +
              'Look for a text message from <strong>Square</strong> with your secure invoice link. Open the link to complete your card payment.' +
            '</p>' +
            '<p style="margin:0;font-size:14px;line-height:1.6;color:#5c544c;">' +
              deliveryTiming + ' You do not need to send a Venmo payment.' +
            '</p>' +
          '</td>' +
        '</tr>' +
      '</table>'
  ...
  const venmoButtonHtml = safeVenmoUrl
    ? '<table role="presentation" cellpadding="0" cellspacing="0" border="0">' +
        '<tr>' +
          '<td style="border-radius:8px;background:#6b5f52;">' +
            `<a href="${safeVenmoUrl}" style="display:inline-block;padding:12px 24px;font-size:15px;color:#ffffff;text-decoration:none;font-weight:600;border-radius:8px;">` +
              'Open in Venmo →' +
            '</a>' +
          '</td>' +
        '</tr>' +
    ...
    '<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f5f1e9;border-radius:10px;margin-bottom:28px;">' +
      '<tr>' +
        '<td style="padding:24px;">' +
          ...
          '</h2>' +
          '<p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#2f2a25;">' +
            `Open Venmo and send <strong>${safeTotal}</strong> to <strong>@${safeVenmoUsername}</strong>.<br>` +
            '<span style="color:#5c544c;font-size:14px;">Include this note so we can match it to your order:</span>' +
          '</p>' +
          '<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:20px;">' +
            '<tr>' +
              '<td style="background:#ffffff;border:1px solid #eee3d2;padding:10px 14px;border-radius:6px;font-family:\'SFMono-Regular\',Menlo,Monaco,Consolas,monospace;font-size:15px;color:#2f2a25;">' +
                safeOrderNumber +
              '</td>' +
            '</tr>' +
          '</table>' +
...
export async function sendOrderEmails(order) {
  ...
  // Card requests are invoiced by hand, so the payment instructions in both
  // emails must not point at Venmo.
  const isCardOrder = order.paymentMethod === 'card';
  const customerPhone = formatPhoneNumber(order.customer?.phone);
  const paymentInstructions = isCardOrder
    ? `Look for a text message from Square with your secure invoice link${
        customerPhone ? ` at ${customerPhone}` : ''
      }. Open it to complete your card payment. It will arrive within one business day.`
    : `Open Venmo and send ${total} to @${venmoUsername} with the note: ${order.orderNumber}`;
  const paymentHtml = renderBuyerPaymentHtml({
    isCardOrder,
    total,
    venmoUsername,
    venmoUrl,
  ...
  const buyerParams = {
    ...
    shipping,
    total,
    venmo_link: isCardOrder ? '' : venmoUrl,
    venmo_username: isCardOrder ? '' : venmoUsername,
    venmo_instructions: paymentInstructions,
    payment_method: isCardOrder ? 'card' : 'venmo',
  ...
  const adminParams = {
    ...
    payment_method: isCardOrder ? 'card' : 'venmo',
    payment_instructions: isCardOrder
      ? `CARD REQUEST - send this customer a Square invoice by text${customerPhone ? ` at ${customerPhone}` : ''}.`
      : paymentInstructions,
    customer_phone: customerPhone,
    phone: customerPhone,
    site_url: siteUrl,
