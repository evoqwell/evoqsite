...
export async function sendOrderEmails(order) {
  ...
  const paymentInstructions = isCardOrder
    ? `You asked to pay by credit card. We'll text a secure payment link${
        customerPhone ? ` to ${customerPhone}` : ''
      } within one business day. No Venmo payment is needed.`
  ...
  const adminParams = {
    ...
    payment_instructions: isCardOrder
      ? `CARD REQUEST - invoice this customer${customerPhone ? ` at ${customerPhone}` : ''}.`
      : paymentInstructions,
    customer_phone: customerPhone,
    phone: customerPhone,
    site_url: siteUrl,
